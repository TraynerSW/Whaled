...
class ThemeRepository(context: Context) {
    ...
    var currentTheme: AppTheme
        get() {
            var saved = prefs.getString("app_theme", AppTheme.SYSTEM.name) ?: AppTheme.SYSTEM.name
            if (saved == "OLED_BLACK") saved = AppTheme.OLED.name
...
fun WLEDTheme(
    ...
    val colorScheme = when (theme) {
        AppTheme.SYSTEM -> {
            // Label "Système" -> fond dynamique, texte blanc
        ...
        AppTheme.LIGHT -> {
            // Label "Clair" -> Force le fond CLAIR et l'écriture en GRIS TRES FONCE
        ...
        AppTheme.DARK -> {
            // Label "Sombre" -> Force le mode sombre de Material You, texte blanc
            val baseDark = if (dynamicColor && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                val context = LocalContext.current
        ...
        AppTheme.OLED -> {
            // Label "Oled" -> Force le fond NOIR et conserve les couleurs dynamiques de Material You pour les boutons/actions
            val baseOled = if (dynamicColor && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                val context = LocalContext.current
                dynamicDarkColorScheme(context)
            } else {
                OledBlackColorScheme
            }
