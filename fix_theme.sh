...
class ThemeRepository(context: Context) {
    ...
    var currentTheme: AppTheme
        get() {
            // Handle legacy inverted names gracefully by defaulting properly if missing, but we assume Enum entry names are fixed
...
fun WLEDTheme(
    ...
    val colorScheme = when (theme) {
        AppTheme.SYSTEM -> {
            // Material You (système) -> Adapts to system dark/light, but forces text to WHITE
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            WLEDTheme(theme = currentTheme, dynamicColor = true) {
                // Determine logic based on actual theme properties so background matches
